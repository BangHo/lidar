#include "lidar/LiDAR_small_static.hpp"


LiDAR_small_static::LiDAR_small_static()
: Mission("LiDAR_small_static")
{
  pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/Cluster/small_static", 1);
  object_ = this->create_publisher<std_msgs::msg::Float64MultiArray>("/LiDAR/object_cen", 1);
  marker_array_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/Marker/small_static_object",1);

  RCLCPP_INFO(this->get_logger(), "LiDAR_small_static is ready");

}

int LiDAR_small_static::get_status_code()
{
    return 11;
}
...
void LiDAR_small_static::callback(const sensor_msgs::msg::PointCloud2::SharedPtr input)
{
    ...
    cropFilter.setMin(Eigen::Vector4f(-0, -3, -0.3, 0)); // x, y, z, i
    cropFilter.setMax(Eigen::Vector4f(15, 4, 1.0, 0));
    ...
    for (int i = 0; i < int(clusters.size()); i++)
    {
      ...
      if ((max_point.y - min_point.y) < 1.5 && (max_point.y - min_point.y) > 0.5 && max_point.z < 0.2)      
      {
